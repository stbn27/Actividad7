...
if (!empty($_GET["id"])) {
    ...
    $query = "DELETE FROM catalogo WHERE id_catalogo = $campo_1";
    $result = pg_query($cn, $query);

    if ($result) {
        $_SESSION['mensaje'] = "<div class='alert alert-warning mt-5' role='alert'>Catalogo con ID <b>$campo_1</b> eliminado con éxito.</div>";
        header("Location: ../catalogo.php");
    } else {
        $_SESSION['mensaje'] = "<div class='alert alert-danger mt-5' role='alert'>Ocurrió un error al intentar eliminar el catalogo <br> <p>Error: " . pg_last_error($cn) . "</p></div>";
        header("Location: ../catalogo.php");
    }
